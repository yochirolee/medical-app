    ...
    <div className="lg:flex grid grid-cols-1 mt-10 relative place-items-end mr-10 md:mr-16  ">
      <div className="slider-bullets lg:mx-auto  flex  w-20 lg:w-2/5 justify-between items-center">
        ...
        {bullets.map((bullet) => (
          <span
            key={bullet.id}
            className={`rounded-full cursor-pointer   bg-gray-500 h-2 w-2 lg:h-3 lg:w-3 ${
        ...
        <i
          className="las la-angle-right pl-2 hidden lg:block cursor-pointer  text-gray-500 lg:text-2xl font-extrabold "
          onClick={() => handleActiveStep(activeStep + 1)}
        ></i>
      </div>
    </div>
